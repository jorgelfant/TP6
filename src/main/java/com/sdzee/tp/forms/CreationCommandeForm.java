...
public final class CreationCommandeForm {
    ...
    public Commande creerCommande(HttpServletRequest request) {
        ...
        if (ANCIEN_CLIENT.equals(choixNouveauClient)) {
            ...
            //Sinon on garde l'ancien mode, pour la validation des champs. L'objet métier pour valider la création
            // d'un client existe déjà, il est donc déconseillé de dupliquer ici son contenu ! À la place, il suffit
            // de passer la requête courante à l'objet métier existant et de récupérer l'objet Client créé.
            CreationClientForm clientForm = new CreationClientForm();
            client = clientForm.creerClient(request);

            //Et très important, il ne faut pas oublier de récupérer le contenu de la map d'erreur créée par
            // l'objet métier CreationClientForm dans la map d'erreurs courante, actuellement vide.
    ...
    //joute un message correspondant au champ spécifié à la map des erreurs --------------------------------------------
